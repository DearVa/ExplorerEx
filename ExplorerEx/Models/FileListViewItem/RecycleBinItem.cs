...
public sealed class RecycleBinItem : FileListViewItem, IFilterable {
	...
	public static void RegisterAllWatchers() {
		...
		foreach (var drive in DriveInfo.GetDrives()) {
			RegisterWatcher(drive.Name[0]);
		}
		Update();
	}

	public static void RegisterWatcher(char driveLetter) {
		...
		if (Watchers[i] == null) {
			...
			watcher.Changed += static (_, _) => Update();
			watcher.Error += static (_, _) => Update();
			try {

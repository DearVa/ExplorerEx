...
public class FolderItem : FileSystemItem {
	...
	private static readonly Dictionary<string, (FolderItem, PathType)> SpecialFolderDictionary = new();
	...
	public static void RegisterSpecialFolder(string fullPath, FolderItem singleton, PathType pathType) {
		if (fullPath.Length == 0 || fullPath[0] != '$') {
			throw new ArgumentException(@"fullPath must start with '$'", nameof(fullPath));
		}

		SpecialFolderDictionary.Add(fullPath, (singleton, pathType));
	}
	...
	public static (FolderItem?, PathType) ParsePath(string path) {
		...
		if (SpecialFolderDictionary.TryGetValue(path, out var specialFolder)) {
		...
		return PathParsers.Select(pathParser => pathParser.Invoke(path)).FirstOrDefault(static result => result.Item1 != null, (null, PathType.Unknown));
	}
	...
	protected FolderItem(string fullPath, string name, ImageSource? defaultIcon) : base(fullPath, name, defaultIcon) {
		IsFolder = true;
	}

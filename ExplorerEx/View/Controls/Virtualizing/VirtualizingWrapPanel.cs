...
public class VirtualizingWrapPanel : VirtualizingPanelBase {
	...
	///     If the value is <see cref="Size.Empty" /> the size of the items got measured by the first realized item.
	/// </summary>
	...
	public bool IsGroupPanel { get; set; }
	...
	protected override Size ArrangeOverride(Size finalSize) {
		...
		var useGroupingArrange = IsGroupPanel && ItemsControl.IsGrouping;

		var childSize = Size.Empty;
		if (!useGroupingArrange) {
			childSize = CalculateChildArrangeSize(finalSize);
		}
	...
	protected override ItemRange UpdateItemRange() {
		...
		if (ItemsOwner is IHierarchicalVirtualizationAndScrollInfo groupItem) {
			...
			case VirtualizationCacheLengthUnit.Pixel: {
				...
				var cacheAfterInPixel = Math.Min(CacheLength.CacheAfterViewport, GetHeight(Extent) - viewportHeight - offsetInPixel);
			...
			var viewportStartPos = GetY(Offset);
			...
			var startRowIndex = GetRowIndex(viewportStartPos);
	...
	private int GetRowIndex(double location) {
		if (IsGroupPanel && ItemsControl.IsGrouping) {
			for (var index = 0; index < InternalChildren.Count; index++) {
				location -= GetHeight(InternalChildren[index].DesiredSize);
				if (location <= 0) {
					return index;
				}
			}

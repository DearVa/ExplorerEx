...
public sealed partial class MainWindow {
	...
	public MainWindow(string? startupPath, bool startUpLoad = true) {
		logger = Service.Resolve<ILoggerService>();
		...
		Width = Math.Min(configure.LoadInt("WindowWidth", 1200), screenWidth);
		Height = configure.LoadInt("WindowHeight", 800);

		var left = configure.LoadInt("WindowLeft");
		var top = configure.LoadInt("WindowTop");
		...
		if (configure.LoadBoolean("WindowMaximized")) {
	...
	private void OnSidebarSelectionChanged(object sender, SelectionChangedEventArgs e) {
		void UpdateSidebarColumnDefinitionWidth() {
			var width = configure.LoadDouble("SidebarWidth");
		...
		if (loaded) {
			if (sidebar.SelectedIndex == -1) {
				configure.Save("IsSidebarOpen", false);
				SidebarColumnDefinition.Width = new GridLength(0, GridUnitType.Auto);
			} else {
				configure.Save("IsSidebarOpen", true);
				configure.Save("SidebarIndex", sidebar.SelectedIndex);
				UpdateSidebarColumnDefinitionWidth();
			}
		} else {
			if (configure.LoadBoolean("IsSidebarOpen")) {
				sidebar.SelectedIndex = configure.LoadInt("SidebarIndex");
				UpdateSidebarColumnDefinitionWidth();
			} else {
	...
	private void SidebarSplitter_OnPreviewMouseMove(object sender, MouseEventArgs e) {
		if (mouseDownOnSidebar) {
			...
			if (width > SidebarMinWidth / 2) {
				if (!isOpen) {
					var index = configure.LoadInt("SidebarIndex");
			...
			if (isOpen) {
				configure.SaveToBuffer("SidebarWidth", width);
			}
		}
		e.Handled = true;
	}
	...
	protected override void OnLocationChanged(EventArgs e) {
		...
		configure.SaveToBuffer("WindowLeft", (int)Left);
		configure.SaveToBuffer("WindowTop", (int)Top);
	}

	protected override void OnRenderSizeChanged(SizeChangedInfo sizeInfo) {
		base.OnRenderSizeChanged(sizeInfo);
		if (sizeInfo.WidthChanged && (int)sizeInfo.NewSize.Width != (int)sizeInfo.PreviousSize.Width) {
			configure.SaveToBuffer("WindowWidth", (int)sizeInfo.NewSize.Width);
		}
		if (sizeInfo.HeightChanged && (int)sizeInfo.NewSize.Height != (int)sizeInfo.PreviousSize.Height) {
			configure.SaveToBuffer("WindowHeight", (int)sizeInfo.NewSize.Height);
		}
	}
	...
	protected override void OnPreviewDragEnter(DragEventArgs e) {
		if (DataObjectContent.IsWindowDragEnter) {  // 只有首次进入Window才触发
	...
	protected override void OnPreviewDragLeave(DragEventArgs e) {
		if (DataObjectContent.IsWindowDragLeave) { // 只有当真正离开了窗口，才取消显示
	...
	private static void IsMaximized_OnChanged(DependencyObject d, DependencyPropertyChangedEventArgs e) {
		...
		window.configure.Save("WindowMaximized", isMaximized);
		if (isMaximized) {
